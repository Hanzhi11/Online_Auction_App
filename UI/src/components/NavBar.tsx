...
function NavBar() {
    ...
    const {overLayContent, updateOverLayContent} = useContext(OverLayContentContext)
    ...
    const linkStyle = className('leading-9 hover:cursor-pointer flex items-center', {
    ...
    return (
        <nav className={navStyle}>
            {links.map(({icon, text, to, onClick}, index) => {
                return (<Link className={linkStyle} key={index} to={to} onClick={onClick}>
                    {icon}
