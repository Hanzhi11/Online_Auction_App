...
function Auctions() {
    ...
    const content = cards.map((card, index) => {
        let fullAddress = card.streetNumber + ' ' + card.street + ', ' + card.suburb
        if (card.unitNumber) {
            fullAddress = card.unitNumber + '/' + fullAddress
        }

        const auctionDate = new Date(card.auctionDate)
        ...
        return (
            <div className="transition-width duration-0 xl:duration-500 ease-in-out w-[315px] h-52 relative md:w-[345px] lg:w-[294px] xl:w-[360px] overflow-hidden group rounded-md text-white bg-cover" style={{ backgroundImage: `url(${card.mainPhoto})` }} key={index}>
                ...
                <span className={"bg-white font-medium px-2 py-1 absolute bottom-2.5 right-2 text-black " + defaultOpacityTransition}>{card.agency}</span>
                <div className={"absolute inset-0 grid grid-rows-6 grid-cols-2 bg-indigo-900 bg-opacity-70 text-sm font-medium h-full p-5 pb-12 " + overLayOpacityTransition}>
                    ...
                    <p className="text-green-500 col-span-2">{card.agency}</p>
                    <p>Auction Listing</p>
                    <a className={anchorStyle}>VIEW</a>
                    <p>Register to BID</p>
    ...
    return (
        <IconContext.Provider value={{ color: 'white', size: '1.2rem', className: 'inline stroke-[20]' }}>
            <SectionContainer header='Upcoming Auctions'>
                ...
                <button className="block mx-auto mt-5 bg-green-500 rounded-full w-10 h-10" onClick={()=>{}}><FaPlus /></button>
            </SectionContainer>
